...
- `src/core/MerlinCCU.cpp`: startup, main loop, screen saver decisions, display
  present scheduling.
- `src/core/console_controller.cpp`: menu routing, softkey dispatch, and
  input-event coordination; delegates page-family logic to the split
  controller modules below (issue #44).
- `src/core/pinter_controller.cpp`, `src/core/alert_controller.cpp`,
  `src/core/calendar_controller.cpp`, `src/core/settings_controller.cpp`,
  `src/core/status_controller.cpp`: page-family logic split out of
  `console_controller.cpp` (issue #44) -- Pinter brew workflow, system alert
  generation/ack/clear (`alert_controller::sync()`), calendar filtering, the
  Settings page and runtime-config selection helpers, and manager-status
  ingestion (`set_*_status()`), respectively. `include/core/
  console_controller_internal.h` exposes the small set of controller-owned
  softkey-label scratch buffers and helpers these modules need without making
  them public API.
- `src/display/screens.cpp`: shared drawing primitives (detail rows, centred
  text, label wrapping, graph plotting) and the `draw_menu_screen()` dispatch.
  All page families have been split into their own files (issue #45,
  complete): `status_screens.cpp`, `weather_screens.cpp`,
  `air_traffic_screens.cpp`, `calendar_screens.cpp`, `shares_screens.cpp`,
  `pinter_screens.cpp`, `settings_screens.cpp`, `alert_screens.cpp`, and
  `screen_banners.cpp` (top status banner).
...
| 2026-08-13 | Follow-up to #44 (already closed): extracted the last named-but-skipped candidate, status snapshot ingestion, into `status_controller.h`/`.cpp` -- the `set_*_status()` change-detection/copy logic for Wi-Fi, time, Home Assistant, MQTT, air traffic, share market, environment sensor, keypad monitor, main-loop load, heap, stack, and display-timing snapshots. Unlike the other three modules, these functions are part of the public `console_controller.h` API, so `console_controller.cpp`'s `set_*_status()` bodies became thin forwarders (`if (!status_controller::set_wifi_status(g_console_state, wifi_status)) return false; update_softkeys_from_state(); return true;`) rather than disappearing outright -- `update_softkeys_from_state()` is private dispatch a split-out module can't reach, so the "trigger a redraw" side effect had to stay behind in the forwarder while the change-detection/copy logic moved. `console_controller.cpp`: 2508 -> 2280 lines (4817 -> 2280 across all five staged steps this session, a 53% reduction). | User asked whether anything from the original candidate list had been missed after #44 was already closed; status ingestion was the one area explicitly named in the issue text that hadn't been touched, so it was worth finishing even though the issue's acceptance bar had already been met without it. | This is very likely the natural stopping point: what remains in `console_controller.cpp` (shared softkey/lamp-index primitives, keypad LTRS text-entry decode, the `update_softkeys_from_state()`/`apply_softkey_route()`/`handle_button_event()` dispatch chain, and the public API surface) has no more self-contained, independently-reusable chunks left to pull out without fragmenting the dispatcher itself -- which would just relocate tightly-coupled code, not separate concerns, the same reasoning the 2026-07-08 entry used to defer this file's split in the first place. |
| 2026-08-14 | Investigated issue #104 (static RAM at 97.6%, ~12KB free) with `arm-none-eabi-nm --print-size --size-sort` against the linked `.elf`. Found the biggest reducible items were not what the Resources page's aggregate figure suggested: the two display raster buffers (38% of all static RAM) are load-bearing display pipeline, not a quick win; `ram_heap` (65KB) turned out to be lwIP's internal `MEM_SIZE` pool, not newlib's heap; `memp_memory_PBUF_POOL_base` (37KB) is lwIP's `PBUF_POOL_SIZE`; and the three ~16-24KB HTTP response buffers (`home_assistant_manager`, `air_traffic_manager`, `web_config_server`) have no sizing rationale in code, with `air_traffic_manager`'s already documented as having been *widened* once after a real overflow. None of these four had real usage data to size against (lwIP's `MEM_STATS`/`MEMP_STATS` were off), unlike `share_price_manager`'s prior 16KB->4KB reduction (#42), which was justified by a provable bound from a known-compact JSON contract. Rather than guess -- this project's incident history includes both #19's network lockup and the ADS-B buffer's own prior overflow -- added instrumentation instead: enabled `MEM_STATS`/`MEMP_STATS` in `lwipopts.h` (+276 bytes RAM) and added high-water-mark tracking (`g_response_peak_len` / `g_response_ever_truncated`) to all four response buffers, all reported via the existing `PERIODIC_LOG`/`debug_logging::kEnablePeriodicSerialOutput` gate (zero cost when disabled, as today). | User asked whether the Resources page's static-RAM WARN was worth worrying about; it is (2.4% headroom, two open issues planning new allocation), but the obvious-looking reduction targets turned out to have documented reasons they're sized the way they are, or no way to verify a smaller size is safe without hardware. Guessing wrong risks reintroducing a failure mode this project has already hit twice. | Real numbers now require one real-hardware bring-up session: flip `kEnablePeriodicSerialOutput` on, run the device through its normal network-heavy paths (weather/calendar fetch, ADS-B fetch, share fetch, web config page loads) for a while, and read back `lwip_mem`/`pbuf_pool` peaks and each buffer's `peak`/`ever_truncated` line over serial. Only then does shrinking any of these four numbers move from a guess to a measured decision. If `air_traffic_manager`'s `g_response_ever_truncated` comes back true even at 16KB, that buffer needs widening again, not shrinking. |
| 2026-08-14 | Added host-test coverage for the five `console_controller.cpp`-split modules (`pinter_controller`, `alert_controller`, `calendar_controller`, `settings_controller`, `status_controller`) that had none since #44 (issue #78). None were host-buildable as-is: `pico/error.h` usage in two files was replaced with a local `kNoErrorCode` constant (same pattern #71 used); `console_controller.cpp`, `pinter_controller.cpp`, and the rest link-required three symbols with no host-safe implementation -- `make_default_console_state()` (real one reads flash linker symbols, #71), `pinter_store::save()`, and `config_manager::save()` (only `settings()` was stubbed before). Added `tests/host/stubs/console_model_stub.cpp` (bare zero-initialized `ConsoleState`, since real test fixtures are built by hand, not via this function) and `stubs/pinter_store_stub.cpp` (no-op success); extended `stubs/config_manager_stub.cpp` with a stateful `save()` that mirrors the real implementation's observable contract (updates in-memory settings immediately via the same `config_persistence::sanitize_settings()`, omitting only the flash write) rather than a fixed default. 70 new test cases across 5 new files. | User asked whether anything had been missed on a prior "increase test scope" pass (#78); an earlier audit comment on that issue had scoped it down to #71/#72 (both now closed), but the five new controller modules this session's #44 work created had no coverage of their own -- a genuine, currently-uncovered gap achievable without hardware, unlike the issue's original "automate via the web preview" framing. | Found and fixed three real test-authoring bugs the new tests caught by actually running: two status_controller change-detection tests whose "before" value coincidentally matched `ConsoleState`'s zero-initialized default (so the first call was never actually a change); a Pinter flush test not accounting for `g_pinter_save_pending` being a file-scope static shared across every test in the binary; and `settings_controller::handle_timeout_edit_event()`'s digit decoding depending on `console_controller.cpp`'s private `g_console_state.letter_mode` rather than the `ConsoleState&` parameter the function nominally takes -- undetectable from a unit test without going through the full public button-event API, so that path stays untested and is called out in the test file. `select_weather_source()`/`select_relative_time_zone()`/`select_screen_saver()`/`toggle_*_enabled()` have the same hidden-global characteristic (they mutate through `persist_runtime_config_change()` into `g_console_state`, not the passed state) and are also not covered, noted in `test_settings_controller.cpp`. |
| 2026-08-14 | Restored the above Decision Log entry after it was silently dropped by a merge-conflict resolution ("take incoming") when PR #107 was merged into `main` -- PR #105's and #106's entries had been added at the same insertion point (immediately before this table's blank template row) after PR #107's branch was created, so the conflict was a same-location table insert. All of PR #107's actual code (5 new test files, 3 stub files, `CMakeLists.txt` entries) landed correctly; only this prose paragraph was lost. | User reported resolving a merge conflict by taking the incoming change; given that phrase alone doesn't say which side "incoming" was or which file, checked `origin/main` directly rather than assuming the resolution was safe. | Worth remembering for any file multiple concurrently-open PRs append to at a fixed location (this Decision Log table, changelogs, etc.): a same-spot conflict is likely, and "take incoming" during a *branch* update (merging `main` into a PR branch, as opposed to merging the PR into `main`) can discard the PR's own not-yet-merged addition at that spot without it being obvious from the resolution alone. Diffing the merged result against each source side (not just trusting the conflict resolution) is the reliable check. |
| 2026-08-14 | Documentation audit pass for issue #3 (recurring repository hygiene review). Fixed staleness accumulated from this session's #44/#45 controller/screen splits and #9/#42 share-feed work: this file's own "Current Runtime Shape" section still described `console_controller.cpp` as owning alert state and `screens.cpp` as having Calendar/Shares/Pinter/Settings/Alerts "pending" a split that finished weeks ago; `README.md` still described an admin-password web-config gate that had been removed, a "pending" share feed that issue #42 implemented, non-configurable watched shares that issue #9 made configurable, and a "Host Tests" section describing only keypad/alert coverage against a suite that has since grown to include golden-image rendering and the five split controller modules; `docs/alerts.md` still referenced the now-nonexistent `console_controller::sync_system_alerts()` and had its Alert Catalogue and Missing Telemetry tables empty since the doc was created -- filled both from `alert_controller.cpp`'s current 17-code `AlertCode` enum and `sync()` body. AGENTS.md was read in full and found to need no changes -- healthy, within the issue's own target size, no stale references found. | User picked issue #3 explicitly, with an explicit "do not close it" instruction reinforcing the issue's own documented history (it was previously closed by mistake and had to be reopened over a missed 2026-07-04 "keep open" note). This pass targeted concrete, verifiable staleness only, per the issue's stated preference to avoid recommending change for its own sake. | Issue #3 remains open by design as a recurring item; this is one pass, not a closing one. Not yet done from the issue's own five-pass list: a README "deflation" pass (trimming rather than just correcting), a hardware-docs reconciliation pass, and a documentation-map pass. `alert_controller.cpp`'s `DisplayPipelineLag` alert has a stale-in-a-different-sense placeholder: its comment says frame-timing telemetry isn't available, but `DisplayTimingStatus.present_skipped_count` was added under issue #71 and the alert still hardcodes `false` -- flagged in the new Missing Telemetry table rather than fixed here, since wiring it up is a behavioural change, not a documentation one. |
| 2026-08-14 | Audited issue #18 (Complete Pinter Scheduling Assistant) against its own acceptance criteria: full-flow state machine (`pinter_scheduling.cpp`), dock/fridge capacity gating with an on-screen block-reason message (not just a squeezed softkey hint), bounds-checked day-adjustment validation, wear-leveled/CRC-validated flash persistence (`pinter_store.cpp`/`pinter_persistence.cpp`), a read-only web `/pinter` activity timeline, and existing host test coverage (issue #78) -- no missing steps, validation gaps, or placeholder behaviour found in the Pinter-specific code. Fixed one narrowing-conversion compiler warning found during the review (`pinter_controller.cpp`'s `confirm_start()`, `size_t`->`uint8_t` via an explicit `static_cast`, since `brew_catalogue_index()`'s return is already bounds-checked against the `uint8_t`-sized catalogue). Closed #18 as already complete. | User picked #18 expecting real completion/hardening work; a full read against the issue's own acceptance criteria found nothing to harden, so reported that back rather than inventing unnecessary changes, and the user confirmed closing it. | One adjacent, genuine placeholder was found but intentionally left alone since it's outside #18's Pinter-only scope: the `Shares` page's "REMOVE" softkey is permanently wired to `SoftKeyRoute::None, false` (dead, unimplemented). Worth its own issue if useful. |
| 2026-08-14 | Fixed a gap in #106's #104 RAM instrumentation: `home_assistant_manager::g_response`'s peak-usage tracking was only ever surfaced through the pre-existing `direct_weather_log()` helper, which is gated behind its own `kEnableDirectWeatherSerialDiagnostics` flag *and* explicitly skips logging whenever `g_active_weather_source == WeatherSource::HomeAssistant` -- so a user actually running HA as their weather/entity source (the most fully-integrated path) would get zero peak-size data for this buffer no matter what, since every `RequestKind` (`ProbeApi`/`FetchTrackedEntity`/`FetchWeatherEntity`/`FetchWeatherForecast`/`FetchWeatherDailyForecast`/`FetchSunEntity`/`PublishSelfEntity`) cycles through this one shared buffer via `advance_request_kind()`. Added an unconditional `PERIODIC_LOG` at the top of `handle_http_status()` (the single dispatch point every request kind's response passes through) reporting `g_response_peak_len` regardless of weather source or request kind. | User picked up issue #104 to actually run the real-hardware session PR #106 was left waiting on; before sending them off to collect data, re-read the instrumentation and found this buffer's logging would silently produce nothing useful for their setup, which uses Home Assistant. Better to catch an instrumentation gap before a hardware session than have them come back with a suspiciously-missing number. | Not yet verified on hardware -- same caveat as the rest of this issue's work. `air_traffic_manager`'s and `share_price_manager`'s peak logging were re-checked and don't have the same gap (their `PERIODIC_LOG` calls are unconditional already). |
| YYYY-MM-DD |  |  |  |
