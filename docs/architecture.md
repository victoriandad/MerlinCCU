...
| 2026-08-13 | Fixed a proportions regression in the web Display Preview page (`web_config_server.cpp`'s `build_preview_page()`) left over from issue #79's pixel-for-pixel canvas fix: `--rect-key-w`/`--rect-key-h`/`--font-small`/`--font-large` (top-row and A-Z keypad matrix buttons) were never scaled down when the display panel shrank ~25% (358x450 -> 270x338) to render at native resolution, so the keys read as oversized; `.display-shell` also had no `justify-content`, so the now-narrower display+softkey column left-aligned within `.display-bay` while the (justify-content:center) top-row/keypad above and below it stayed centered, reading as the whole preview shifted left. Scaled the four CSS variables by the same ~0.75 factor (78/56/12/18px -> 59/42/9/14px) and added `justify-content:center` to `.display-shell`. | User reported the regression with a screenshot after this session's #72 work merged; root-caused via a targeted history search of `web_config_server.cpp` rather than guesswork, tracing it to commit `02eb726` (#79) which scaled the display/softkey geometry but missed the keypad key-size variables and the shell's centering. | Not visually verified in a browser this session -- no way to run the Pico's actual HTTP server or a dev-server equivalent for this embedded page locally; the fix is a straightforward proportional CSS scale plus one `justify-content` addition, verified by firmware compiling cleanly, but ask the user to confirm the preview looks right again before considering issue #79 fully closed. |
| YYYY-MM-DD |  |  |  |
